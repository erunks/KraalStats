...
exports.up = function(knex, Promise) {
	return knex.schema.createTable('matches', (table) => {
		...
		table.integer('fighter_one_id').unsigned().notNullable();
		table.foreign('fighter_one_id').references('fighters.id');
		table.integer('stocks_taken_by_player_one').unsigned().defaultTo(0);
		...
		table.integer('fighter_two_id').unsigned().notNullable();
		table.foreign('fighter_two_id').references('fighters.id');
		table.integer('stocks_taken_by_player_two').unsigned().defaultTo(0);
